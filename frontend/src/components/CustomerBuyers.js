...
import {
  FilterPanel, KpiCard, LoadingState, EmptyState,
...
// ─── Gráfica simple ───────────────────────────────────────────────────────────
const SimpleBar = ({ labels, values, color }) => (
  <Box sx={{ height: 260 }}>
    <Bar
...
const CustomerBuyers = ({ accentColor = ACCENT }) => {
  ...
  const COLS = [
    ...
    { id: 'total_purchase_amount', label: 'Total comprado',  numeric: true,  bold: true,
      render: r => formatCurrency(r.total_purchase_amount), color: () => GREEN },
  ];
          ...
          {isMobile
            ? visible.map(c => (
                <ClientCard key={c.client_id} name={c.client_name} id={c.client_id}
                  primary={c.total_purchase_amount} primaryLabel="Total comprado" color={GREEN} />
              ))
