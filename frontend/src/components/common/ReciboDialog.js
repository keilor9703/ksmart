...
.grand{font-size:20px;font-weight:700;color:#0891B2;}
...
<div class="header">
  ${empresa?.logo_base64 ? `<img src="${empresa.logo_base64}" style="height:52px;margin-bottom:10px;display:block;margin:0 auto 10px;">` : `<div style="width:52px;height:52px;border-radius:12px;background:#0891B218;display:flex;align-items:center;justify-content:center;margin:0 auto 10px;font-size:22px;">🏪</div>`}
...
const ReciboDialog = ({ open, onClose, venta, empresa, vendedor }) => {
  ...
  return (
    ...
    >
      {/* ── Top bar ─────────────────────────────────── */}
      <Box sx={{ height: 4, bgcolor: '#0891B2', flexShrink: 0 }} />

      {/* ── Header ──────────────────────────────────── */}
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2.5, py: 1.5, flexShrink: 0 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Box sx={{ width: 36, height: 36, borderRadius: 1.5, bgcolor: '#0891B218', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Receipt sx={{ color: '#0891B2', fontSize: 20 }} />
          </Box>
          <Box>
      ...
      <Box sx={{ px: 2.5, py: 2, display: 'flex', gap: 1.5, flexShrink: 0, flexWrap: 'wrap' }}>
        ...
        <Button
          ...
          sx={{
            ...
            bgcolor: '#0891B2', '&:hover': { bgcolor: '#e65520' },
          }}
        >
          Imprimir / PDF
        </Button>
