...
export default function GestionSaaS() {
  ...
  const fetchEmpresas = async () => {
    try { const { data } = await apiClient.get('/superadmin/empresas'); setEmpresas(data); } catch (e) { toast.error('Error al cargar inquilinos'); }
  };

  const fetchCatalogoPlanes = async () => {
    try { const { data } = await fetchPlanesAdmin(); setPlanesCatalog(data); } catch (e) { console.error("Error cargando planes", e); }
  };

  const fetchHistorialPagos = async () => {
    try { const { data } = await apiClient.get('/superadmin/historial-pagos'); setPagos(data); } catch (e) { console.error("Error cargando historial de pagos"); }
  };
  ...
  // ── Funciones de Asignación de Planes ──
    ...
    <Box sx={{ width: '100%' }}>
      ...
      {tabValue === 0 && (
        <Paper sx={{ borderRadius: 3, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', overflow: 'hidden', bgcolor: isMobile ? 'transparent' : 'background.paper' }}>
          {!isMobile ? (
            ...
            <Box sx={{ p: { xs: 0, sm: 2 } }}>
              {empresas.length === 0 ? (
                <Box sx={{ textAlign: 'center', py: 6, color: 'text.secondary' }}><Business sx={{ fontSize: 48, mb: 1, opacity: 0.3 }} /><Typography>No hay empresas registradas</Typography></Box>
              ) : (
                empresas.map(emp => <EmpresaCard key={emp.id} empresa={emp} onToggleStatus={handleToggleStatus} onOpenPlan={handleOpenAsignarPlan} />)
              )}
            </Box>
          )}
        </Paper>
          ...
          {planesCatalog.length === 0 ? (
            <Grid item xs={12}>
              <Paper sx={{ p: 5, textAlign: 'center', borderRadius: 3 }}>
                ...
                <Typography sx={{ fontWeight: 700, color: 'text.secondary' }}>No has creado planes de suscripción.</Typography>
                <Button variant="outlined" onClick={handleOpenCreatePlan} sx={{ mt: 2 }}>Crear mi primer Plan</Button>
              </Paper>
            </Grid>
          ) : (
      ...
      <Dialog open={openCatalogDialog} onClose={() => setOpenCatalogDialog(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography sx={{ fontWeight: 700, fontSize: 17 }}>{editingPlanId ? 'Editar Plan' : 'Crear Nuevo Plan'}</Typography>
        ...
        <form onSubmit={handleSubmitPlan}>
          <DialogContent dividers>
            <Stack spacing={2.5}>
              ...
              <TextField label="Código Interno" required size="small" value={formPlan.codigo_interno} onChange={e => setFormPlan({...formPlan, codigo_interno: e.target.value.toLowerCase().replace(/ /g, '_')})} disabled={!!editingPlanId} helperText="Sin espacios, ej: plan_basico" />
              ...
              <FormControlLabel control={<Switch checked={formPlan.is_active} onChange={e => setFormPlan({...formPlan, is_active: e.target.checked})} color="primary" />} label="Plan Visible para Clientes" />
            </Stack>
          </DialogContent>
          <DialogActions sx={{ p: 2 }}>
            <Button onClick={() => setOpenCatalogDialog(false)} variant="outlined" sx={{ borderRadius: 2 }}>Cancelar</Button>
            <Button type="submit" variant="contained" disabled={loading} sx={{ bgcolor: BLUE, borderRadius: 2 }}>Guardar Plan</Button>
          </DialogActions>
        </form>
      </Dialog>

      {/* Modal: Asignar Plan a Empresa */}
      <Dialog open={openPlanDialog} onClose={() => setOpenPlanDialog(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
        <DialogTitle sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <Typography sx={{ fontWeight: 700, fontSize: 16 }}>Suscripción: {empresaSeleccionada?.nombre}</Typography>
          ...
          <DialogContent dividers>
            <Stack spacing={3}>
              <TextField select label="Seleccionar Plan o Acción" value={formAsignarPlan.plan_selector} onChange={handleSelectPlanChange} fullWidth size="small">
                <MenuItem value="trial">Volver a Trial (Prueba)</MenuItem>
                <MenuItem value="premium">Premium Genérico (Manual)</MenuItem>
                <Divider sx={{ my: 1 }} />
