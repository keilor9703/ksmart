...
import {
  ...
  MoreHoriz, Refresh, ReceiptLong, MoneyOff
} from '@mui/icons-material';
import { toast } from 'react-toastify';
import apiClient from '../api';
import { formatCurrency } from '../utils/formatters';
import CurrencyField from './CurrencyField';
import QuickCreateModal from './QuickCreateModal'; // Importamos tu Modal Reutilizable
...
export default function Caja() {
  ...
  // ── Estados Originales (Cierre Caja) ──
  ...
  // ── Estados Nuevos (Gastos / Terceros) ──
  ...
  const handleCerrarCaja = async () => {
    ...
    try {
      ...
      setOpenDialog(false); setEfectivoFisico(0); setObservaciones('');
      fetchPreview(); fetchHistorial();
    } catch (err) {
  ...
  const handleRegistrarGasto = async (e) => {
    ...
    try {
      ...
      fetchGastos(); 
      fetchPreview(); // El preview se debe recalcular para restar el dinero de caja
    } catch (err) {
      toast.error('Error al registrar el gasto');
    } finally { setSubmitting(false); }
  };

  // ── Handlers Inline Creation ──
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1, flexWrap: 'wrap', gap: 2 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Box>
            ...
            <Typography sx={{ fontSize: 12, color: 'text.secondary' }}>Arqueo, cortes y gastos menores</Typography>
          </Box>
        </Box>
      </Box>
      ...
      <Paper sx={{ borderRadius: 3, boxShadow: '0 2px 12px rgba(0,0,0,0.06)', overflow: 'hidden', mb: 3 }}>
        <Tabs value={tab} onChange={(_, v) => setTab(v)} sx={{ px: 2, borderBottom: '1px solid', borderColor: 'divider', '& .MuiTab-root': { fontWeight: 600, fontSize: 13, textTransform: 'none', minHeight: 52 }, '& .MuiTabs-indicator': { backgroundColor: ACCENT, height: 3, borderRadius: 3 }, '& .Mui-selected': { color: `${ACCENT} !important` } }}>
      ...
      <TabPanel value={tab} index={0} sx={{ pt: 0 }}>
        <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2, gap: 1 }}>
          <Button variant="outlined" startIcon={<Refresh />} onClick={fetchPreview} size="small" sx={{ borderRadius: 2, fontWeight: 600, borderColor: 'divider', color: 'text.secondary' }}>Actualizar</Button>
          <Button variant="contained" startIcon={<PointOfSale />} onClick={() => setOpenDialog(true)} size="small" sx={{ background: `linear-gradient(135deg, ${ACCENT}, #ff9a62)`, boxShadow: `0 4px 14px rgba(255,96,32,0.3)`, borderRadius: 2, fontWeight: 600 }}>Cerrar Caja</Button>
        </Box>
        ...
        {loadingPreview ? (
          ...
          <>
            <Grid container spacing={1.5} sx={{ mb: 2 }}>
              <Grid item xs={6} sm={3}><KpiCard label="Ingresos del día" value={formatCurrency(preview.total_dia)} icon={<TrendingUp />} color={ACCENT} sub={preview.fecha} /></Grid>
              <Grid item xs={6} sm={3}><KpiCard label="Efectivo (Caja)" value={formatCurrency(preview.efectivo)} icon={<AttachMoney />} color={GREEN} /></Grid>
              <Grid item xs={6} sm={3}><KpiCard label="Transferencias" value={formatCurrency(preview.transferencia)} icon={<AccountBalance />} color={BLUE} /></Grid>
              <Grid item xs={6} sm={3}><KpiCard label="Gastos (Salidas)" value={formatCurrency(preview.total_gastos)} icon={<MoneyOff />} color={RED} /></Grid>
            </Grid>

            <Paper sx={{ p: 2.5, borderRadius: 3, boxShadow: '0 2px 8px rgba(0,0,0,0.06)', mb: 2 }}>
              <Typography sx={{ fontWeight: 700, fontSize: 14, mb: 1.5 }}>Dinero disponible por método — hoy</Typography>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', pt: 1.5, pb: 1 }}>
                <Typography sx={{ fontWeight: 700, fontSize: 14, color: RED }}>(-) Total Egresos/Gastos</Typography>
                <Typography sx={{ fontWeight: 800, fontSize: 14, color: RED }}>{formatCurrency(preview.total_gastos)}</Typography>
              </Box>

              <Box sx={{ display: 'flex', justifyContent: 'space-between', pt: 1.5, borderTop: '2px solid', borderColor: 'divider' }}>
                <Typography sx={{ fontWeight: 700, fontSize: 14 }}>Saldo Neto (Ingresos - Gastos)</Typography>
                <Typography sx={{ fontWeight: 800, fontSize: 16, color: ACCENT }}>{formatCurrency(preview.total_dia - preview.total_gastos)}</Typography>
              </Box>

              {(preview.ventas_contado > 0 || preview.abonos_cartera > 0) && (
                <Box sx={{ mt: 2, pt: 1.5, borderTop: '1px dashed', borderColor: 'divider' }}>
                  <Typography sx={{ fontSize: 11, fontWeight: 600, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.6, mb: 1 }}>Por origen de ingreso</Typography>
                  ...
                  {historial.map(c => {
                    ...
                    return (
                      <TableRow key={c.id} hover>
                        <TableCell sx={{ fontSize: 11, whiteSpace: 'nowrap' }}>{new Date(c.fecha).toLocaleDateString('es-CO', { day: '2-digit', month: 'short', year: 'numeric' })}</TableCell>
                        ...
                        <TableCell sx={{ fontWeight: 700, color: difColor }}>{dif === 0 ? '—' : dif > 0 ? `+${formatCurrency(dif)}` : formatCurrency(dif)}</TableCell>
                        <TableCell>
      ...
      {/* ── TAB 1: REGISTRO DE GASTOS / EGRESOS ── */}
      <TabPanel value={tab} index={1} sx={{ pt: 0 }}>
        <Grid container spacing={3}>
            ...
            <Paper sx={{ p: 3, borderRadius: 3, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              ...
              <Box component="form" onSubmit={handleRegistrarGasto}>
                <Stack spacing={2}>
                  ...
                  {/* Inline Creation: Proveedor / Beneficiario */}
                  <Autocomplete
                    options={terceros}
                    ...
                    noOptionsText={
                      <Box sx={{ py: 0.5 }}>
                        ...
                        <Button size="small" variant="contained" fullWidth startIcon={<Add />} onClick={() => openQuickCreate(terceroInput)}
                          ...
                          endAdornment: (
                            <>
                              ...
                              <Tooltip title="Crear nuevo proveedor/beneficiario">
                                <IconButton size="small" onClick={() => openQuickCreate(terceroInput)} sx={{ color: '#3B82F6', p: 0.5 }}>
                                  <Add fontSize="small" />
                                </IconButton>
                              </Tooltip>
                            </>
                      ...
                      {['Efectivo', 'Transferencia', 'Tarjeta'].map(opt => (
                        <Chip key={opt} label={opt} onClick={() => setGastoMetodo(opt)}
                          sx={{ fontWeight: 600, fontSize: 12, borderRadius: 1.5,
                            ...
                            border: '1.5px solid', borderColor: gastoMetodo === opt ? ACCENT : 'divider',
                  ...
                  <Button type="submit" variant="contained" disabled={submitting}
                    sx={{ mt: 1, background: `linear-gradient(135deg, ${RED}, #f87171)`, boxShadow: `0 4px 14px rgba(239,68,68,0.3)`, borderRadius: 2, fontWeight: 600 }}>
          ...
          <Grid item xs={12} md={7}>
            <Paper sx={{ p: 3, borderRadius: 3, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              <Typography sx={{ fontWeight: 700, fontSize: 15, mb: 2 }}>Historial de Gastos</Typography>
              {loadingGastos ? (
      ...
      {/* ── Dialog Cierre Caja ── */}
      <Dialog open={openDialog} onClose={() => !submitting && setOpenDialog(false)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: 3, overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' } }}>
        <Box sx={{ height: 4, bgcolor: ACCENT }} />
        <DialogTitle sx={{ pb: 1, pt: 2.5, pr: 6 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            <Box sx={{ width: 38, height: 38, borderRadius: 2, bgcolor: `${ACCENT}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: ACCENT }}><PointOfSale /></Box>
            <Box>
          ...
          <IconButton size="small" onClick={() => setOpenDialog(false)} disabled={submitting} sx={{ position: 'absolute', right: 12, top: 16, color: 'text.secondary' }}><Close fontSize="small" /></IconButton>
        </DialogTitle>

        <DialogContent sx={{ pt: 1 }}>
          <Paper sx={{ p: 1.5, mb: 2, borderRadius: 2, bgcolor: 'action.hover', boxShadow: 'none', border: 'none' }}>
            <Typography sx={{ fontSize: 11, fontWeight: 600, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.6, mb: 1 }}>Resumen del sistema</Typography>
            {[
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
              <Typography sx={{ fontSize: 13, fontWeight: 700 }}>Total en Caja Esperado</Typography>
              <Typography sx={{ fontSize: 13, fontWeight: 800, color: ACCENT }}>{formatCurrency(preview?.efectivo || 0)}</Typography>
            </Box>
          </Paper>

          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            <CurrencyField label="Efectivo físico contado" value={efectivoFisico} onChange={setEfectivoFisico} helperText="Cuenta el dinero en caja y digita el total" />
            {efectivoFisico > 0 && (
              <Alert severity={diferencia === 0 ? 'success' : diferencia > 0 ? 'info' : 'error'} sx={{ borderRadius: 2, fontSize: 13 }}>
                {diferencia === 0 ? '✓ Cuadre exacto' : diferencia > 0 ? `Sobrante: ${formatCurrency(Math.abs(diferencia))}` : `Faltante: ${formatCurrency(Math.abs(diferencia))}`}
              </Alert>
            )}
        ...
        <DialogActions sx={{ px: 3, pb: 2.5, pt: 1, gap: 1 }}>
          <Button onClick={() => setOpenDialog(false)} disabled={submitting} variant="outlined" size="small" sx={{ borderRadius: 2, fontWeight: 600, borderColor: 'divider', color: 'text.secondary', flex: 1 }}>Cancelar</Button>
          <Button onClick={handleCerrarCaja} disabled={submitting} variant="contained" size="small" startIcon={submitting ? <CircularProgress size={14} color="inherit" /> : <CheckCircle sx={{ fontSize: 16 }} />} sx={{ borderRadius: 2, fontWeight: 600, flex: 1, background: `linear-gradient(135deg, ${ACCENT}, #ff9a62)`, boxShadow: `0 4px 14px rgba(255,96,32,0.3)`, color: '#fff' }}>
