...
import {
  ...
  TableHead, TableRow, Paper, Button, Chip, Grid, TableSortLabel
} from '@mui/material';
...
const SalesCard = ({ item }) => (
  <Paper sx={{ p: 2.5, mb: 2, borderRadius: 3, boxShadow: '0 2px 10px rgba(0,0,0,0.06)' }}>
    <Typography sx={{ fontWeight: 700, fontSize: 14, mb: 0.5 }}>{item.product_name}</Typography>
    <Typography sx={{ fontSize: 12, color: 'text.secondary', mb: 1.5 }}>#{item.product_id}</Typography>
...
const ProductSales = ({ accentColor = ACCENT }) => {
  ...
  const handleSort  = (col) => {
    setOrder(prev => orderBy === col && prev === 'desc' ? 'asc' : 'desc');
    setOrderBy(col);
  };
  ...
  const COLS = [
    ...
    { id: 'total_quantity_sold',   label: 'Cant. vendida', numeric: true,  bold: true },
  ...
  const renderSection = (title, sorted, visible, showKey, color) => (
    <Box sx={{ mb: 4 }}>
      <SectionTitle badge={`${sorted.length} ítems`} color={color}>{title}</SectionTitle>
      ...
      {sorted.length > 5 && (
        <Button onClick={() => setShowAll(p => ({ ...p, [showKey]: !p[showKey] }))}
          sx={{ mt: 1.5, fontWeight: 600, color, fontSize: 12 }}>
        ...
        <>
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={12} sm={4}>
              <KpiCard label="Productos distintos vendidos" value={salesData.productos.length} icon={<ShoppingCart />} color={accentColor} />
            </Grid>
            <Grid item xs={12} sm={4}>
