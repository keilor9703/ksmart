  ...
  return (
    <Paper sx={{ p: 2.5, mb: 2, borderRadius: 3, boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
      ...
      <Grid container spacing={1} sx={{ mb: 1.5 }}>
        ...
        <Grid item xs={6}>
          <Box sx={{ textAlign: 'center', p: 1, borderRadius: 2, bgcolor: 'action.hover' }}>
            <Typography sx={{ fontSize: 10, color: 'text.secondary', mb: 0.2 }}>Operador</Typography>
            <Typography sx={{ fontSize: 13, fontWeight: 700 }}>{orden.operador?.username || 'N/A'}</Typography>
          </Box>
        </Grid>
      </Grid>
      ...
      <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end', mt: 1.5 }}>
        ...
        {(orden.estado === 'Borrador' || orden.estado === 'Rechazada') && (
          <>
            ...
            <Tooltip title="Enviar a revisión">
              <IconButton size="small" onClick={() => onSend(orden.id)} sx={{ color: YELLOW, bgcolor: `${YELLOW}12`, borderRadius: 1.5 }}>
                <Send fontSize="small" />
              </IconButton>
            </Tooltip>
          </>
...
const OrdenesTrabajo = ({ user }) => {
  ...
  const fetchAccumulatedTotal = (filters = {}) => {
    ...
    apiClient.get('/ordenes-trabajo/total', { params })
      ...
      .catch(() => toast.error("Error al cargar total"));
  };

  const fetchOrdenesParaAprobar = () =>
    apiClient.get('/ordenes-trabajo/?estado=En revisión')
      .then(res => setOrdenesParaAprobar(res.data))
  ...
  const handleAddProducto = () =>
    setAddedProductos([...addedProductos, { id: Date.now() + Math.random(), producto: null, cantidad: 1, precioUnitario: 0 }]);

  const handleRemoveProducto = (id) => {
    setAddedProductos(addedProductos.filter(p => p.id !== id));
    setProductoInputs(prev => { const next = { ...prev }; delete next[id]; return next; });
  };
  ...
  const handleAddServicio = () =>
    setAddedServicios([...addedServicios, { id: Date.now() + Math.random(), servicio: null, cantidad: 1, precioUnitario: 0 }]);

  const handleRemoveServicio = (id) =>
    setAddedServicios(addedServicios.filter(s => s.id !== id));
  ...
  const handleEnviarRevision = (id) => {
    setOrdenAction({ action: 'enviar', id, message: '¿Enviar esta orden a revisión? No podrá editarla después.' });
  ...
  const handleApprove = async (id) => {
    ...
    if (!ok) return;
    setOrdenAction({ action: 'aprobar', id, message: '¿Aprobar esta orden? Se registrará la venta automáticamente.' });
    setShowConfirmDialog(true);
  };
    ...
    try {
      ...
      (orden.productos || []).forEach((linea) => {
        ...
        if (disponible < requerido) {
          faltantes.push({ nombre: prod.nombre, requerido, disponible });
        }
      });
  ...
  return (
    <Box sx={{ width: '100%' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Box>
            ...
            <Typography sx={{ fontSize: 13, color: 'text.secondary' }}>
              Gestión de órdenes de servicio y producción
            </Typography>
          </Box>
        </Box>
        <Button
      ...
      {/* ── KPIs (solo en tab historial) ── */}
      {tabValue === 1 && (
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={6} sm={4}>
            <KpiCard label="Total órdenes" value={statsOrdenes.total} icon={<Receipt />} color={ACCENT} />
          </Grid>
          <Grid item xs={6} sm={4}>
            <KpiCard label="En revisión" value={statsOrdenes.pendientes} icon={<PendingActions />} color={YELLOW} />
          </Grid>
          <Grid item xs={12} sm={4}>
            <KpiCard label="Total acumulado" value={formatCurrency(accumulatedTotal)} icon={<TrendingUp />} color={GREEN} />
          </Grid>
        </Grid>
      )}
            ...
            <Box component="form" onSubmit={handleSubmit}>
              ...
              {user?.role?.name === 'Admin' && (
                <Autocomplete
                  ...
                  onChange={(_, v) => setSelectedOperatorForForm(v)}
              ...
              <Box sx={{ mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  ...
                  <Button size="small" startIcon={<Add />} onClick={handleAddProducto}
                ...
                <Stack direction="column" spacing={1.5}>
                  {addedProductos.map(p => (
                    <Box key={p.id} sx={{
                      ...
                      gap: 1.5, alignItems: isMobile ? 'stretch' : 'center',
                      ...
                      <Autocomplete
                        ...
                        value={p.producto}
                        onChange={(_, v) => {
                          handleProductoChange(p.id, 'producto', v);
                          handleProductoChange(p.id, 'precioUnitario', v ? v.precio : 0);
                        }}
                        ...
                        sx={{ flex: 2, minWidth: isMobile ? '100%' : 220 }}
                      />
                      <TextField
                        type="number"
                        label="Cantidad"
                        size="small"
                        value={p.cantidad}
                        onChange={e => handleProductoChange(p.id, 'cantidad', parseFloat(e.target.value))}
                      ...
                      <TextField
                        ...
                        onChange={e => handleProductoChange(p.id, 'precioUnitario', parseFloat(e.target.value))}
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
                        <Typography sx={{ fontWeight: 700, fontSize: 14, color: ACCENT, minWidth: 90 }}>
              ...
              <Box sx={{ mb: 3 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                  ...
                  <Button size="small" startIcon={<Add />} onClick={handleAddServicio}
                  ...
                  {addedServicios.map(s => (
                    <Box key={s.id} sx={{
                      ...
                      <Autocomplete
                        ...
                        value={s.servicio}
                        onChange={(_, v) => {
                          handleServicioChange(s.id, 'servicio', v);
                          handleServicioChange(s.id, 'precioUnitario', v ? v.precio : 0);
                        }}
                        renderInput={(params) => (
                          <TextField {...params} label="Servicio" size="small" placeholder="Busca..." />
                        )}
                        sx={{ flex: 2, minWidth: isMobile ? '100%' : 220 }}
                      />
                      <TextField
                        ...
                        size="small"
                        value={s.cantidad}
                        onChange={e => handleServicioChange(s.id, 'cantidad', parseFloat(e.target.value))}
                        sx={{ width: isMobile ? '100%' : 110 }}
                      />
                      <TextField
                        type="number"
                        ...
                        onChange={e => handleServicioChange(s.id, 'precioUnitario', parseFloat(e.target.value))}
                        sx={{ width: isMobile ? '100%' : 140 }}
                        InputProps={{
                          startAdornment: <InputAdornment position="start">$</InputAdornment>
                        }}
                      />
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1 }}>
                        <Typography sx={{ fontWeight: 700, fontSize: 14, color: BLUE, minWidth: 90 }}>
                          {formatCurrency(s.cantidad * s.precioUnitario)}
                        </Typography>
                        <Tooltip title="Quitar">
                          <IconButton
                            size="small"
              ...
              <Box sx={{
                ...
                <Paper sx={{
                  ...
                  bgcolor: `${ACCENT}0D`, border: `1.5px dashed ${ACCENT}60`,
                  boxShadow: 'none', minWidth: 200,
                }}>
                  <Typography sx={{ fontSize: 11, color: 'text.secondary', mb: 0.5 }}>
                    Total Orden
                  </Typography>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  ...
                  >
                    {editingOrden ? 'Actualizar Orden' : 'Guardar Borrador'}
                  </Button>
                </Box>
              </Box>
            </Box>
            ...
            <Grid container spacing={2} sx={{ mb: 2.5 }}>
              ...
              <Grid item xs={12} sm={3}>
                <Autocomplete
                  ...
                  renderInput={(params) => <TextField {...params} label="Filtrar por cliente" fullWidth size="small" />}
                />
              </Grid>
              {user?.role?.name === 'Admin' && (
                <Grid item xs={12} sm={3}>
                  <Autocomplete
                    ...
                    onChange={(_, v) => setSelectedOperator(v)}
                    renderInput={(params) => <TextField {...params} label="Filtrar por operador" fullWidth size="small" />}
                  />
                </Grid>
              )}
            </Grid>
            ...
            {isMobile ? (
                ...
                {ordenes.length === 0 ? (
                  ...
                  [...ordenes].reverse().map(orden => (
                    <OrdenCard
                      key={orden.id}
                      orden={orden}
                      onEdit={(o) => { setEditingOrden(o); setTabValue(0); }}
                      onView={(o) => { setSelectedOrden(o); setShowDetailDialog(true); }}
                      onSend={handleEnviarRevision}
              ...
              <TableContainer sx={{ borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      {['ID', 'Cliente', 'Operador', 'Productos', 'Servicios', 'Total', 'Estado', 'Fecha', 'Acciones'].map(h => (
                      ...
                      [...ordenes].reverse().map(orden => (
                        <TableRow key={orden.id} hover>
                          ...
                          <TableCell sx={{ fontSize: 11, whiteSpace: 'nowrap' }}>
                            {new Date(orden.fecha_creacion + 'Z').toLocaleDateString()}
                          </TableCell>
                          <TableCell>
                            <Tooltip title="Ver detalles">
                              <IconButton size="small" onClick={() => { setSelectedOrden(orden); setShowDetailDialog(true); }}
                            ...
                            {(orden.estado === 'Borrador' || orden.estado === 'Rechazada') && (
                              <>
                                <Tooltip title="Editar">
                                  <IconButton size="small" onClick={() => setEditingOrden(orden)}
                                    sx={{ color: ACCENT, '&:hover': { bgcolor: `${ACCENT}12` } }}>
                                    <Edit fontSize="small" />
                                  </IconButton>
                                </Tooltip>
                                <Tooltip title="Enviar a revisión">
                                  <IconButton size="small" onClick={() => handleEnviarRevision(orden.id)}
                                    sx={{ color: YELLOW, '&:hover': { bgcolor: `${YELLOW}12` } }}>
                                    <Send fontSize="small" />
                                  </IconButton>
                                </Tooltip>
                              </>
                            ...
                            {user?.role?.name === 'Admin' && (orden.estado === 'Aprobada' || orden.estado === 'Rechazada') && (
                              <Tooltip title="Cerrar orden">
                                <IconButton size="small" onClick={() => handleCloseOrder(orden)}
              ...
              {isMobile ? (
                <Box>
                  {ordenesParaAprobar.length === 0 ? (
                    <Box sx={{ textAlign: 'center', py: 6, color: 'text.secondary' }}>
                      ...
                      <Typography>No hay órdenes pendientes de aprobación</Typography>
                    </Box>
                  ) : (
                ...
                <TableContainer sx={{ borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
                  <Table size="small">
                    <TableHead>
                      <TableRow>
                        {['ID', 'Cliente', 'Operador', 'Total', 'Fecha', 'Acciones'].map(h => (
                          <TableCell key={h}>{h}</TableCell>
                        ))}
                      </TableRow>
                    </TableHead>
                        ...
                        ordenesParaAprobar.map(orden => (
                          <TableRow key={orden.id} hover>
                            ...
                            <TableCell>
                              ...
                              <Tooltip title="Aprobar">
                                <IconButton size="small" onClick={() => handleApprove(orden.id)}
                                  sx={{ color: GREEN, '&:hover': { bgcolor: `${GREEN}12` } }}>
                                  <CheckCircle fontSize="small" />
                                </IconButton>
                              </Tooltip>
                              <Tooltip title="Rechazar">
                                <IconButton size="small" onClick={() => handleOpenRejectDialog(orden)}
