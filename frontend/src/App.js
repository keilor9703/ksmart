...
const ACCENT         = '#FF6020';       // naranja vibrante
const ACCENT_SOFT    = '#FFF0E9';
...
const Home = () => (
  ...
  >
    <Box
      ...
      alt="Ksmart360"
      sx={{ width: 120, height: 120, borderRadius: 4, boxShadow: '0 8px 32px rgba(0,0,0,0.12)' }}
    />
    <Box sx={{ textAlign: 'center' }}>
      <Typography
        variant="h4" fontWeight={700}
        sx={{ fontFamily: "'Plus Jakarta Sans', sans-serif", mb: 1 }}
      >
        Bienvenido a Ksmart360
      </Typography>
      <Typography variant="body1" color="text.secondary">
        La nueva forma de gestionar tu negocio de forma inteligente
      </Typography>
    </Box>
  </Box>
);
    ...
    >
      <ListItemIcon
        sx={{
          minWidth: 0,
          mr: expanded ? 1.5 : 0,
          color: active ? ACCENT : (item.color || '#94a3b8'),
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', py: 1.5,
        ...
        {user?.role?.name === 'Admin' && (
          <>
            <SidebarItem
              expanded={expanded}
              item={{ text: 'Administración', icon: <AdminPanelSettings />, color: '#a78bfa' }}
              ...
              <List disablePadding>
                {adminMenuItems.map(sub => (
                  <ListItemButton
                    ...
                    onClick={mobile ? onClose : undefined}
                    sx={{
                      pl: 5, pr: 2, py: 0.75, mx: 1, mb: 0.25,
                      borderRadius: 2,
                      backgroundColor: isActive(sub.path) ? SIDEBAR_ACTIVE : 'transparent',
                  ...
                  >
                    ...
                    <ListItemText
                      primary={sub.text}
                      primaryTypographyProps={{
                        fontSize: 13, color: isActive(sub.path) ? '#fff' : '#94a3b8',
                        fontFamily: "'Plus Jakarta Sans', sans-serif",
                      }}
                    />
                  </ListItemButton>
            ...
            {expanded && (
              <Typography
                sx={{
                  ...
                  color: '#475569', letterSpacing: 1.2,
...
// ─── TopBar ────────────────────────────────────────────────────────────────────
const TopBar = ({
  sidebarExpanded, isMobile, onMobileMenuOpen,
  mode, onThemeToggle, onLogout, user,
  anchorEl, openMenu, onMenuOpen, onMenuClose,
}) => {
  const location = useLocation();
  ...
  return (
    <Box
      component="header"
      sx={{
        position: 'fixed', top: 0, right: 0, zIndex: 1100,
        left: isMobile ? 0 : (sidebarExpanded ? SIDEBAR_FULL : SIDEBAR_MINI),
        height: 60,
        display: 'flex', alignItems: 'center',
        px: { xs: 2, md: 3 },
        backgroundColor: mode === 'dark' ? '#0d1117' : '#fff',
        borderBottom: `1px solid ${mode === 'dark' ? 'rgba(255,255,255,0.06)' : '#E5E7EB'}`,
        gap: 2,
        transition: 'left 0.25s ease',
      }}
    >
      {isMobile && (
        <IconButton onClick={onMobileMenuOpen} size="small">
          <MenuIcon />
        </IconButton>
      )}
            ...
            {!isMobile && (
              <Box
                ...
                sx={{
                  ...
                  height: '100vh',
                  width: sidebarExpanded ? SIDEBAR_FULL : SIDEBAR_MINI,
