  ...
  return (
    ...
    <Paper
      ...
      sx={{
        display: 'flex',
        borderRadius: 2.5,
        border: demorado ? `1.5px solid ${RED}60` : `1px solid ${alpha(estadoConfig.color, 0.2)}`,
    ...
    >
      <Box sx={{ width: 4, flexShrink: 0, bgcolor: demorado ? RED : estadoConfig.color, borderRadius: '8px 0 0 8px' }} />
      <Box sx={{ p: 1.8, flexGrow: 1, position: 'relative' }}>
        <TipoIcon sx={{ position: 'absolute', top: 8, right: 8, fontSize: 48, opacity: 0.12, color: estadoConfig.color, pointerEvents: 'none' }} />
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.8 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.4 }}>
            ...
            <Typography sx={{ fontSize: 22, fontWeight: 900, fontFamily: "'Courier New', monospace", letterSpacing: 4, color: 'text.primary' }}>
              {orden.placa}
            </Typography>
          </Box>
        </Box>
        <Typography sx={{ fontSize: 11, color: 'text.secondary', mb: 1, mt: -0.5 }}>{orden.tipo_vehiculo || '—'}</Typography>
    ...
    <Box sx={{ p: { xs: 1, md: 2 }, maxWidth: 1300, mx: 'auto' }}>
      ...
      <Grid container spacing={2.5}>
        ...
        >
          ...
          {/* Trabajador y Cliente consolidados */}
          <Paper sx={{ p: 2.5, borderRadius: 3, mb: 2, border: '1px solid', borderColor: 'divider' }}>
              ...
              {isAdmin && trabajadoresFiltrados.length > 0 && (
                <Box sx={{ flex: 1 }}>
                  ...
                  <Autocomplete
                    ...
                    getOptionLabel={t => t.nombre_completo || t.username || ''}
                    value={operadorObj}
                    onChange={(_, v) => { setOperadorObj(v); setOperadorId(v ? v.id : ''); }}
                    renderInput={params => <TextField {...params} label="Lavador" placeholder="Seleccionar..." />}
                  />
                </Box>
              )}
              <Box sx={{ flex: 1 }}>
                <SectionLabel step={isAdmin && trabajadoresFiltrados.length > 0 ? 4 : 3}>Cliente (opcional)</SectionLabel>
                <Autocomplete
                  size="small"
                  options={clientes}
